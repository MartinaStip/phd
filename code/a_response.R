...
missing_sum = missing %>% 
  ...
  impute_labs %>% 
  mutate(yvar = case_when(str_detect(name, "info_") ~ n_miss_mc$info,
                          str_detect(name, "situation_") ~ n_miss_mc$situation,
                          TRUE ~ n_missing/nrow(missing) * 100),
         xvar = case_when(is.na(lab) ~ label, 
